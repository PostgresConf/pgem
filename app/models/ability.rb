class Ability
  ...
  def not_signed_in
    ...
    can :quickbuy, Event do |event|
      event.ticket && event.scheduled? && event.program.conference.registration_open? && event.program.conference.digital? && !event.ended?
    end
  end
  ...
  def signed_in_with_organizer_role(user)
    ...
    can :manage, ConferenceTeamMember, conference_id: conf_ids_for_organizer
